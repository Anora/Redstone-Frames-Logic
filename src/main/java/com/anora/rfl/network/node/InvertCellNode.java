...
/**
 * RedPower 2 Invert Cell (sim model).
 *
 * Two crossing lines:
 *  - bottom line (channel 0) passes through unchanged
 *  - top line (channel 1) is affected by bottom:
 *
 * top_out = top_in OR NOT(bottom_in)
 * bottom_out = bottom_in
 *
 * Output is bundledOut with channels 0 and 1 set accordingly.
 */
public final class InvertCellNode extends PositionedNode {

    public InvertCellNode(NetPos pos, DelayStore store) {
        super(pos, store);
    ...
    public boolean evaluate() {
        ...
        BundledSignal next = BundledSignal.ALL_OFF
                ...
                .with(1, topOut);  // inverted interaction
