package com.anora.rfl.network.node;

import com.anora.rfl.core.BundledSignal;
import com.anora.rfl.core.SignalValue;
import com.anora.rfl.network.NetPos;
import com.anora.rfl.network.util.DelayStore;

/**
 * RedPower 2 Non-Invert Cell (sim model).
 *
 * Two crossing lines:
 *  - bottom line (channel 0) passes through unchanged
 *  - top line (channel 1) is affected by bottom:
 *
 * top_out = top_in OR bottom_in
 * bottom_out = bottom_in
 *
 * Output is bundledOut with channels 0 and 1 set accordingly.
 */
public final class NonInvertCellNode extends PositionedNode {

    public NonInvertCellNode(NetPos pos, DelayStore store) {
        super(pos, store);
        this.singleOut = SignalValue.OFF;
        this.bundledOut = BundledSignal.ALL_OFF;
    }

    @Override
    public void beginPass() {
        // no-op
    }

    @Override
    public boolean evaluate() {
        boolean bottom = bundledIn.isOn(0);
        boolean topIn = bundledIn.isOn(1);

        boolean topOut = topIn || bottom;

        BundledSignal next = BundledSignal.ALL_OFF
                .with(0, bottom)  // bottom pass-through
                .with(1, topOut); // non-inverting interaction

        if (!next.equals(bundledOut)) {
            bundledOut = next;
            return true;
        }
        return false;
    }
}
